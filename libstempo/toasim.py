...
def quantize_fast(times, flags=None, dt=1.0):
    ...
    avetoas = N.array([N.mean(times[ind]) for ind in bucket_ind], "d")
    if flags is not None:
        aveflags = N.array([flags[ind[0]] for ind in bucket_ind])
...
def add_jitter(psr, ecorr, flagid=None, flags=None, coarsegrain=0.1, seed=None):
    ...
    if flags is None:
        ...
        t, f, U = quantize_fast(N.array(psr.toas(), "d"), N.array(psr.flagvals(flagid)), dt=coarsegrain)
...
def add_glitch(psr, epoch, amp):
    ...
    psr.stoas[:] -= amp * heaviside(psr.toas() - epoch) * (psr.toas() - epoch) * 86400.0
...
def add_cgw(
    ...
    phat = N.array([N.sin(ptheta) * N.cos(pphi), N.sin(ptheta) * N.sin(pphi), N.cos(ptheta)])
...
def add_ecc_cgw(
    ...
    if check:
        ...
        y = eu.solve_coupled_ecc_solution(F, e0, gamma0, l0, mc, q, N.array([0.0, toas.max()]))
    ...
    # earth term #####
    splus, scross = eu.calculate_splus_scross(nharm, mc, dist, F, e0, toas, l0, gamma0, gammadot, inc)
    ...
    if psrTerm:
        ...
        y = eu.solve_coupled_ecc_solution(F, e0, gamma0, l0, mc, q, N.array([0.0, tp.min()]))
        ...
        if N.any(y):
            ...
            splusp, scrossp = eu.calculate_splus_scross(nharm, mc, dist, Fp, ep, toas, lp, gp, gammadotp, inc)
...
def extrap1d(interpolator):
    ...
    def ufunclike(xs):
        return N.array(list(map(pointwise, N.array(xs))))
