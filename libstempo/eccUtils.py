...
def calculate_splus_scross(nmax, mc, dl, F, e, t, l0, gamma, gammadot, inc):
    ...
    # intermediate terms
    sp = np.sin(phasem) / (n * omega - 2 * gammadot) + np.sin(phasep) / (n * omega + 2 * gammadot)
    sm = np.sin(phasem) / (n * omega - 2 * gammadot) - np.sin(phasep) / (n * omega + 2 * gammadot)
    cp = np.cos(phasem) / (n * omega - 2 * gammadot) + np.cos(phasep) / (n * omega + 2 * gammadot)
    cm = np.cos(phasem) / (n * omega - 2 * gammadot) - np.cos(phasep) / (n * omega + 2 * gammadot)

    splus_n = -0.5 * (1 + np.cos(inc) ** 2) * (an * sp - bn * sm) + (1 - np.cos(inc) ** 2) * cn * np.sin(phase1)
